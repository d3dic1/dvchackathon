...
}).superRefine(({ gameSlug, score }, ctx) => {
  if (score > SCORE_LIMITS[gameSlug]) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['score'], message: 'Score exceeds game limit' })
  }
})
...
router.post('/', async (req: Request, res: Response) => {
  ...
  // Optional: if user is signed in (via optionalClerkAuth middleware on server), attach their identity
  const userId = req.auth?.userId

  try {
    const service = await getScoreService()
    const entry = await service.saveScore(gameSlug, deviceId, score, run.runId, userId)
